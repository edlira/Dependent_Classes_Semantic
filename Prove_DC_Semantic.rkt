...
(define-language DC
  (p    ::= (e (c ...)))
   (CBody ::= empty e)
  (c    ::= (class cx (f : T) ... : T extends (CT ...) CBody))
  (e    ::= v this (e ! f) (new cx (f = e) ...))
  (v    ::= (obj cx (f = v) ...))
  
  (CT   ::= (cx (f : T) ...))    ; regular class types
  (P    ::= this (P ! f))        ; paths, and distinguishes it from the expression 
  ...
  (cx   ::= x)
  (f    ::= x)
  (x    ::= (variable-except class obj : = ! new this extends empty )  )
)
...
(define red (reduction-relation DC #:domain p
   ...
   (--> ((in-hole C (new cx (f = v) ...))                       (c_1 ... c c_2 ...))
        ...
        ;the same same matching declaration as in the vaidas thesis, just find one, but not the most specific
        (where (class cx (f : T) ... : T_cres extends (CT ...) e) c)
        (judgment-holds (type-norms (cx (f : v) ...) (T ...) (NT ...)))
        (judgment-holds (sub-types ((v NT) ...) (c_1 ... c c_2 ...)))
        "object creation")
))
;here we have a non-determinism and it is up to our decision how to solve it, or just through an exception.
...
;(define-judgment-form DC
;  #:mode (lookupCBody I O)
;  #:contract (lookupCBody NT CBody)
;  
;  [ (where (class cx (f : T_c) ... : T_res extends (CT_1 ... CT_ext CT_2 ...) CBody_c) cx)
;
;   ---------------------------------------- "retrive the body of the class (if necessary)"
;   (lookupCBody cx CBody_c )]
;) 
...
; START THE POINT WHERE WE HAVE TO DEFINE THE UNIQUENES AND COMPLETNESS
; COMPLETMESS TO SEE IF IT MAKES SENSE IN ALL OF ITS SUBCLASEES
;UNIQUENESS TO MAK A DECISSION REGARDING THE DIAMOND SHAPE AND DISPATCH STRATEGY 

;(define-judgment-form DC
;  #:mode (complete I I)
;  #:contract (complete CBody (c ... ))
;   [;(where (cx ...) c)
; ;(where (class cx (f : T_c) ... : T_res extends (CT_1 ... CT_ext CT_2 ...) CBody_c) (cx ...))
;; (sub-type (cx (f : T_c) ...) (cx (f_2 : NT_2) ...) (c ... ))
;   (where (class cx (f : T_c) ... : T_res extends (CT_1 ... CT_ext CT_2 ...) e) c )
;
; (where  (CBody_c := e) cx)
;   ----------------------------------- "complete equal classes "
;   (complete (lookupCBody cx CBody) (c_1 ... c c_2...) )]
;
;[;(where (cx ...) c...)(sub-type (cx (f : NT_1) ...) NT_2 (c_1 ... c c_2 ...))  
;(where (class cx (f : T_c) ... : T_res extends (CT_1 ... CT_ext CT_2 ...) CBody_c) c)
;(where (CBody_c := e) cx )
; ----------------------------------- "complete classes "
;   (complete (lookupCBody cx CBody) (c_1 ... c c_2...))]
;
...
;(define-judgment-form DC
;  #:mode (unique I I)
;  #:contract (unique CBody (c ... ))
;
;[(where (class cx (f1 : T1_c) ... : T_rejs extends (CT1_1 ... CT1_ext CT1_2 ...) CBody1_c) c)
; (where (class cx (f2 : T2_c) ... : T_rejs extends (CT2_1 ... CT2_ext CT2_2 ...) CBody2_c) c)
; (sub-type (cx (f_1 : NT_1) ...) (cx (f_2 : NT_2) ...) (c ...))
; (where (CBody1_c := e) cx)
; ----------------------------------- "unique classes "
;   (unique (lookupCBody cx CBody) (c_1 ... c c_2...))]
; )
...
(define-judgment-form DC
 #:mode (unique I I O )
 #:contract (unique c (c ...) bool)
  
 [ (unique (class cx(f : T) ... : T_r extends (CT ...) e) p #f )
   (found (cx(f : T) ... : T_r extends (CT ...) empty) #t)]
    
 [ (unique (val m = (lookup m p (m(f : T) ..._1 : T_r extends (CT ...) e) ) ...) (c ...)  #f )
 (where (cx(f_1 : T_1) ..._1 : T_res extends (CT1 ...) e_c) m )
;(side-condition, (term (lookup m (c ...) (cx(f_1 : T_1) ..._1 : T_res extends (CT1 ...) e) ) ...))
 ; (found (cx(f_2 : T_2) ..._1 : T_r extends (CT2 ...) e) #t)
  (where (cx(f_2 : T_2) ..._1 : T_res extends (CT2 ...) e_c) m)
  (lookup f_1 ((f_2 : T_2) ...) T_1 ) ...
   (where #f (is_subtype ((T_1 T_2) ...) (c...)))]

  [(unique (class cx(f : T) ... : T_r extends (CT_ext ...) empty) (c_1 ... c c_2...) #t )
  (where (class cx(f_1 : T_1) ..._1 : T_res extends (CT1 ...) e_c) c )
  (where (class cx(f_2 : T_2) ..._1 : T_res extends (CT2 ...) e_c) c)
  (lookup f_1 ((f_2 : T_2) ...) T_1 ) ...
  (sub-type T_1 T_2 (c...)) ...
         ]
)
...
;(define-metafunction DC
;  unique : c p -> string or #t
;[(unique (class cx(f_1 : T_1) ..._1 : T_res extends (CT1 ...) e_c) p)]
;  
;  )

(define-metafunction DC 
  is_subtype : ((T T) ...) p   -> bool
[(is_subtype ((T_a T_b) ...) (c ...)) #f ]
[(is_subtype ((T_a T_b) ...) (c ...)) #t
  (judgment-holds (sub-type T_a T_b ... (c...) ))
  ];(side-condition (not (memq (T_a ...)... (T_b ...) ...)))]
)

;(define-metafunction DC 
;  is_subtype : (( f : T ) ..._1 ) ((f : T) ..._1) p   -> bool
;[(is_subtype ((f : T_a) ..._1) ((f : T_b) ..._1) (c ...)) #f ]
;[(is_subtype ((f : T_a) ..._1) ((f : T_b) ..._1) (c ...)) #t
;  (judgment-holds (sub-type T_a T_b ... (c...) ))
;  ];(side-condition (not (memq (T_a ...)... (T_b ...) ...)))]
;)
 ...
 (define-judgment-form DC
  ...
  [------------------------------ "type-norm this"
   ...
   ; this is as an expression here and not as path, so we have to distinguish between the path and expr
...
;  (sub-type Ctx T_1 T_2 (c ...))]
  ...
  [(lookup f_2 ((f_1 NT_1) ...) NT_12) ...
   ...
   (sub-type (cx (f_1 : NT_1) ..._1) (cx (f_2 : NT_2) ..._1) (c ...))
  ]
...
(define-judgment-form DC
  ...
  [(where (class cx (f : T_f) ... : T_res extends (CT_c ...) e_c)  c)
   ...
   (type-norm (cx (f : NT_e) ...) T_f NT_f) ...    ; normalize declared field types
   (sub-type NT_e NT_f (c_1 ... c c_2 ...)) ...    ; check argument expressions are subtype of parameter (field) types
   (type-norm (cx (f : NT_e) ...) T_res NT_res)    ; normalize return type
  ; (unique (cx (f: NT_e) ...) (c_1 ... c c_2 ...) #t)
   --------------------------------------------------------------- "type new"
...
;   (sub-type NT_e NT_f (c_1 ... c c_2 ...)) ...    ; check argument expressions are subtype of parameter (field) types
;   (type-norm (cx (f : NT_e) ...) T_res NT_res)    ; normalize return type
;   (concrete NT_e  p) ... (new cx (f : NT_e) ...)
;   ------------------------------------------------------------------------------------------ "type abs new"
;   (type Ctx (new cx (f = e) ...) NT_res (c_1 ... c c_2 ...))]
 )

;(define-metafunction DC 
;  complete : Ctx e (c ...) -> string or #t
;  [(complete Ctx (new cx (f = e) ...) (c_1 ... c c_2 ...) s)
;   ( (where (class cx (f : T_f) ... : T_res extends (CT_c ...) e_c)  c)
;   (type Ctx e NT_e (c_1 ... c c_2 ...)) ...       ; type argument expressions in current context
;   (type-norm (cx (f : NT_e) ...) T_f NT_f) ...    ; normalize declared field types
;   (sub-type NT_e NT_f (c_1 ... c c_2 ...)) ...    ; check argument expressions are subtype of parameter (field) types
;   (type-norm (cx (f : NT_e) ...) T_res NT_res)    ; normalize return type
;   (lookup ( cx (f : NT_f) ...) cx NT_2)
;   (sub-type (cx (f : NT_e) ...) NT_2 (c_1 ... c c_2 ...))
;   (concrete NT_2  c) (type Ctx (new cx (f = e) ...) NT_res (c_1 ... c c_2 ...)) 
;   ;(sub-type NT_ NT_e (c_1 ... c c_2 ...)) ...
;   ;(concrete NT_es  c) ... (type Ctx (new cx (f = e) ...) NT_res (c_1 ... c c_2 ...)) ...
;   (found NT_2 #f)
;   (where s define s "message-ambigous ") 
;   )]
;  
;  
;  
...
(define-judgment-form DC
  #:mode (complete I I I O)
  #:contract (complete Ctx e (c ...) bool)
  [(complete Ctx (new cx (f = e) ...) (c_1 ... c c_2 ...) #f)
   ;(found (class cx (f : T_f) ... : T_res extends (CT_c ...) e_c) #f)
   (where (class cx (f : T_f) ... : T_res extends (CT_c ...) empty) c)
    ]
   [(complete Ctx (new cx (f = e) ...) (c_1 ... c c_2 ...) #t)
  ( where (class cx (f : T_f) ... : T_res extends (CT_c ...) e_c)  c)
   (type Ctx e NT_e (c_1 ... c c_2 ...)) ...       
   (type-norm (cx (f : NT_e) ...) T_f NT_f) ...    
   (sub-type NT_e NT_f (c_1 ... c c_2 ...)) ...   
   (lookup ( cx (f : NT_f) ...) cx NT_2)
   (sub-type (cx (f : NT_e) ...) NT_2 (c_1 ... c c_2 ...))
   (concrete NT_2  c) (type Ctx (new cx (f = e) ...) NT_res (c_1 ... c c_2 ...)) 
...
;(define-judgment-form DC
;  #:mode (complete I I O O I)
;  #:contract (complete Ctx e NT (bool or string) (c ...))
;  
;  [(where (class cx (f : T_f) ... : T_res extends (CT_c ...) e_c)  c)
;   (type Ctx e NT_e (c_1 ... c c_2 ...)) ...       ; type argument expressions in current context
;   (type-norm (cx (f : NT_e) ...) T_f NT_f) ...    ; normalize declared field types
;   (sub-type NT_e NT_f (c_1 ... c c_2 ...)) ...    ; check argument expressions are subtype of parameter (field) types
;   (type-norm (cx (f : NT_e) ...) T_res NT_res)    ; normalize return type
;   (concrete NT_e  c) ... (type Ctx (new cx (f: NT_e) ...) NT_res (c_1 ... c c_2 ...)) 
;  ; (where NT (sub-types NT_e (type Ctx ( e ! f) NT_o (c ... ))) )
;   ;(sub-type NT_ NT_e (c_1 ... c c_2 ...)) ...
;   ;(concrete NT_es  c) ... (type Ctx (new cx (f = e) ...) NT_res (c_1 ... c c_2 ...)) ...
;    
;   ------------------------------------------------------------------------------------------ "complete equal classes"
;   (complete Ctx (new cx (f = e) ...) NT_res s (c_1 ... c c_2 ...))]
;  
;  [(where (class cx (f : T_f) ... : T_res extends (CT_c ...) e_c)  c)
;   (type Ctx e NT_e (c_1 ... c c_2 ...)) ...       ; type argument expressions in current context
;   (type-norm (cx (f : NT_e) ...) T_f NT_f) ...    ; normalize declared field types
...
(define-judgment-form DC
  ...
  #:contract (type-norm-fix CT ((f : NT) ...) ((f : NT) ...)) ; first one is the sunset of the second one. And this subset has all the normalized types
  ; the known ones and then it is an increasing knowledge 
...
(define-judgment-form DC
  #:mode (concrete I I)
  #:contract (concrete CT p)
  
  [ (where (cx (f : T) ... : T_r extends (CT_ext ... ) e)  p)
   ----------------------------------------------------------- "concrete class"
    (concrete (cx (f : T) ... ) p ) ]
)

(define-judgment-form DC
 #:mode (concreteSet I I O)
  #:contract (concreteSet CT p list)
  
  [(concreteSet cx p l) 
   ;(where (var x = (cx (f : T) ... : T_r extends (CT_ext ... ) e))  p)
    (lookup cx (p) (cx(f : T) ..._1 : T_r extends (CT ...) e))
     ;       )
      (side-condition (term (add (cx) (list l)   )))
  ; (where (cx (f : T) ... : T_r extends (CT_ext ... ) e)  p)
  ]
  ;(lookup m (c ...) (m(f : T) ..._1 : T_r extends (CT ...) e) ) ...) p
   ;----------------------------------------------------------- "concrete class"
    ;(concrete (cx (f : T) ... ) p ) ]
)
...
(define-judgment-form DC
  ...
  [(type-norm-fix (cx (f : T) ...) () ((f_1 : NT_1) ...))
   (lookup f ((f_1 NT_1) ...) NT) ...
   (where NT_c (cx (f : NT) ...))
  ...
  [(type-norm-fix (cx (f : T) ...) () ((f_1 : NT_1) ...))
   ...
   ;complete(cx)
  ;(if ( judgment-holds(unique (cx))) (printf "no-ambiguity") (printf "message-ambiguous"))
   ;(unique cx (c ...) #t)
   (extend-ok NT_c CT_ext (c ...)) ...
   ------------------------------------------------------------------- "abs-class-ok"
   (class-ok (class cx (f : T) ... : T_r extends (CT_ext ...) empty) (c ...))]
  )

(define (program-ok p s)
  (if (judgment-holds (class-ok cx p))
      (begin 
        
        (if (nand (judgment-holds (complete Ctx e p s )))
        (begin 
          ;(where (class cx (f : T_f) ... : T_res extends (CT_c ...) e_c)  p)
          ;(where s (complete Ctx cx NT_res s p))
          (printf "error ~a\n" 
                  s))
         #t)
      )
...
;(define-judgment-form DC
;  #:mode (program-ok I)
;  #:contract (program-ok (c ...))
;  
;  [ (class-ok c (c ...)) ... 
;   ( where (class cx1 (f1 : T1) ... : r1 ) (c ...))
;   (if (complete Ctx (cx1 (f1 : T1) ... ) NT_res #t (c_1 ... c c_2 ...))
;      ( judgment-holds (class-ok cx1 (c ...)))
;      else (printf " message-not-understood"))
;    ( where (class cx2 (f2 : T2) ... : r2 ) (c ...))
;;     (lookup ( cx1) (c ...) ((f : T)...)) 
;;    (type-norm-fix (cx1 (f : T) ...) () ((f_1 : NT_1) ...))
;;    (lookup ( cx2) (c ...) ((f1 : F)...))
;;    (type-norm-fix (cx2 (f1 : F) ...) () ((f1 : FT) ...))
;   ; (lookup f1 ((f_1 NT1_1) ...) NT1) ...
;    ;(lookup f2 ((f_1 NT2_1) ...) NT2) ...
;   (found (and (T1 = T2) (r1 = r2)) #t)
; 
;------------------------------------------------- "program-ok"
;      (program-ok (c ... ))]
;  )
...
(define-judgment-form DC
  #:mode (abs-program-ok I)
  #:contract (abs-program-ok (c ...))
...
;  
;  [
;   (where class cx(f : T)... : T_r)
;   var x = sub-type NT T (c ... )
;   --------------------- "unique"
;   (unique cx)]
;  )
(define-judgment-form DC
  ...
  [(type-norm NT_sub CT_ext (cx_ext (f_ext : NT_ext) ...))
   ...
   (where (class cx_ext (f_cdecl : T_fdecl) ... : T_ext extends (c_ext ...) empty)  c) 
   (type-norm-fix (cx_ext (f_cdecl : T_fdecl) ...) () ((f_decl : NT_fdecl) ...))
...
;(test-true "Nat ab class is ok" (judgment-holds (class-ok ,t-Nat (,t-Nat))))
...
;  (term (new 2DPoint (s = (new 2DSpace))(x = (new Zero))(y = (new Zero)))))
...
;              (term ((obj 2DPoint (s = (obj 2DSpace))(x = (obj Zero))(y = (obj Zero))) (,t-Space ,t-2DSpace ,t-Point ,t-2DPoint ,t-Zero))))
;
;(define t-add2D
;  (term (class add  (s : (2DSpace))
;                    (p1 : (2DPoint(s : (this ! s))(x : (Nat))(y : (Nat))))
;                    (p2 : (2DPoint(s : (this ! s))(x : (Nat))(y : (Nat))))
;          : (2DPoint(s : (this ! s)))
;          extends ()
;          (new 2DPoint (s = (this ! s))
...
;                               (p1 = (new 2DPoint(s = (new 2DSpace))(x = ,(make-nat 0))(y = ,(make-nat 1))))
;                               (p2 = (new 2DPoint(s = (new 2DSpace))(x = ,(make-nat 1))(y = ,(make-nat 0)))))))))
;(test-->> red (make-point-prog
;               (term (new add  (s = (new 2DSpace))
;                               (p1 = (new 2DPoint(s = (new 2DSpace))(x = ,(make-nat 0))(y = ,(make-nat 1))))
;                               (p2 = (new 2DPoint(s = (new 2DSpace))(x = ,(make-nat 1))(y = ,(make-nat 0)))))))
;              (make-point-prog
;               (term (obj 2DPoint(s = (obj 2DSpace))(x = ,(make-nat-val 1))(y = ,(make-nat-val 1))))))
;
;
;(define t-3DSpace
;  (term (class 3DSpace
...
;                    (p1 : (3DPoint(s : (this ! s))(x : (Nat))(y : (Nat))(z : (Nat))))
;                    (p2 : (3DPoint(s : (this ! s))(x : (Nat))(y : (Nat))(z : (Nat))))
;          : (3DPoint(s : (this ! s)))
;          extends ()
;          (new 3DPoint (s = (this ! s))
;                       (x = (new add (n1 = ((this ! p1) ! x)) (n2 = ((this  ! p2) ! x))))
;                       (y = (new add (n1 = ((this ! p1) ! y)) (n2 = ((this ! p2) ! y))))
...
;                               (p1 = (new 3DPoint(s = (new 3DSpace))(x = ,(make-nat 0))(y = ,(make-nat 1))(z = ,(make-nat 2))))
;                               (p2 = (new 3DPoint(s = (new 3DSpace))(x = ,(make-nat 2))(y = ,(make-nat 1))(z = ,(make-nat 0)))))))
;
;(test-true "is prog" (redex-match? DC p (make-3DPoint-prog add-3D-points)))
;(test-->> red (make-3DPoint-prog add-3D-points)
;              (make-3DPoint-prog
;               (term (obj 3DPoint(s = (obj 3DSpace))(x = ,(make-nat-val 2))(y = ,(make-nat-val 2))(z = ,(make-nat-val 2))))))
...
;                               (p1 = (obj 2DPoint(s = (obj 2DSpace))(x = ,(make-nat-val 0))(y = ,(make-nat-val 1))))
;                               (p2 = (obj 3DPoint(s = (obj 3DSpace))(x = ,(make-nat-val 2))(y = ,(make-nat-val 1))(z = ,(make-nat-val 0))))))))
